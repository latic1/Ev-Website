...
export const initiatePasswordResetSchema = object({
    email: string()
        .email()
...
export const resetPasswordSchema = object({
    token: string()
        .required(),
    password: string()
        .min(6, strings.min6Char)
        .max(20, strings.max20Char)
        .required(strings.passwordIsRequired)
        .matches(
            new RegExp(strings.utilStrings.passwordRegex),
            {
                message: strings.passwordDoesntMeet
            }
        ),
    confirmPassword: string()
        ...
        .test('confirmPassword', strings.passwordsMustMatch, function (value) {
...
export const lawyerNetworkApplicationSchema = object({
    email: string()
        .email()
        .required(strings.emailIsRequired)
        .strict(),
    description: string()
        .required(strings.thisFieldIsRequired),
});

export const subscriptionContactSchema = object({
    name: string()
        .email()
    ...
    title: string()
        .email()
        .required(strings.titleIsRequired)
        .strict(),
    email: string()
        .email()
        .required(strings.emailIsRequired)
...
export const signUpSchema = object({
    ...
    email: string()
        ...
        .strict(),
    country: string()
        .required(strings.countryIsRequired)
        .strict()
        .test('country', strings.selectAValidCountry, function (value) {
            return countries.find(country => country.name === value) !== undefined
        }),
    password: string()
        .min(6, strings.min6Char)
        .max(20, strings.max20Char)
...
export const entitySchema = object({
    name: string()
        ...
        .strict(),
    address: string()
        .required(strings.addressIsRequired)
        .strict(),
    type: string()
        .required(strings.typeIsRequired)
        .strict(),
    country: string()
        .required(strings.countryIsRequired)
        .strict()
...
export const referralSchema = object({
    referralCode: string()
        .required(strings.referralCodeIsRequired),
});
